...
class MarhamScraper(BaseScraper):
    """Scraper for Marham doctor profiles.

    This is a template with generic selectors; adjust selectors as needed
    based on the latest Marham HTML structure.
    """
    ...
    def __init__(
        ...
        listing_url: str = "https://www.marham.pk/doctors",
        headless: bool = True,
    ...
    def _extract_profile_links(self, html: str) -> List[str]:
        ...
        for card in soup.select("a[href*='/doctors/']"):
            ...
            if not href:
                continue
            if href.startswith("/"):
                href = "https://www.marham.pk" + href
            if href not in links:
                links.append(href)

        logger.info("Found {} Marham profile links on listing page", len(links))
        return links
    ...
    def _parse_profile(self, html: str, profile_url: str) -> Optional[DoctorModel]:
        ...
        fees_text = clean_text(self._first_text(soup.select_one(".fee, .consultation-fee, .doctor-fee")))
        ...
        rating_text = clean_text(self._first_text(soup.select_one(".rating, .rating-score span")))
        experience = clean_text(self._first_text(soup.select_one(".experience, .doctor-experience")))

        if not name:
            logger.warning("Skipping Marham profile without name: {}", profile_url)
            return None
    ...
    def scrape(self, limit: Optional[int] = None) -> Dict[str, int]:
        """Scrape doctors from Marham.

        Returns stats dict: {"total": int, "inserted": int, "skipped": int}
        """

        logger.info("Starting Marham scraping from {}", self.listing_url)
        ...
        for url in profile_links:
            ...
            logger.info("Scraping Marham profile {} of {}: {}", total, len(profile_links), url)
            self.load_page(url)
            self.wait_for("body")
            html = self.get_html()
            ...
            try:
                ...
                logger.exception("Error parsing/saving Marham profile {}: {}", url, exc)
                skipped += 1

        logger.info(
            "Marham scraping finished. total={}, inserted={}, skipped={}",
            total,
            inserted,
            skipped,
        )
